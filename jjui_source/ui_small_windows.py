...
def window_for_extra_command_character():
    ...
    temp_string += "字体可以调整为 等宽字体，比如：楷体、宋体、…… 等，"+ "\n"
    temp_string += "用等宽字体，感觉正好和原始内容的排版比较匹配，"+ "\n"
    temp_string += "显效的效果比较整齐。"+ "\n"
    temp_string += "注："+ "\n"
    temp_string += "　等宽字体："+ "\n"
    temp_string += "　　一个汉字的宽度 等于 两个(半角)英文字符的宽度 。"+ "\n"
    temp_string += "　　对于 英文 来说，等宽显示可能并不美观，"+ "\n"
    temp_string += "　　比如 i 可以细一点，W 可以相对宽一点，"+ "\n"
    temp_string += "　　所以很多字体 英文字符 的宽度是不同的。"+ "\n"
    temp_string += "\n"
    temp_string += "\n"
