...
class GameList_0(ttk.Frame):
    ...
    def new_func_ui(self,):
        ...
        # 左边一个 frame 放其它的，右边一个 frame 放 垂直 进度条
        ##################################
        #                           #    #
        #                           #    #
        #                           #    #
        ...
        #                           #    #
        ##################################
        self.new_ui_frame_1 = ttk.Frame(parent)
        self.new_ui_frame_2 = ttk.Frame(parent)
        
        parent.rowconfigure(0, weight=1)
        parent.columnconfigure(0, weight=1)
        
        self.new_ui_frame_1.grid( row=0,column=0,sticky=(tk.W,tk.N,tk.S,tk.E,))
        self.new_ui_frame_2.grid( row=0,column=1,sticky=(tk.W,tk.N,tk.S,tk.E,))
        
        #self.new_ui_frame_1.rowconfigure(0, minsize=self.new_var_row_height_for_header)
        self.new_ui_frame_1.rowconfigure(0, weight=0)
        self.new_ui_frame_1.rowconfigure(1, weight=1)
        self.new_ui_frame_1.rowconfigure(2, weight=0)
        self.new_ui_frame_1.columnconfigure(0, weight=1)
        
        self.new_ui_frame_2.rowconfigure(0, weight=1)
        self.new_ui_frame_2.columnconfigure(0, weight=0)
        
        # 把 header 单独放在一个 frame 中
        self.new_ui_frame_for_header = ttk.Frame(self.new_ui_frame_1)
        self.new_ui_frame_for_header.grid( row=0,column=0,sticky=(tk.W,tk.N,tk.S,tk.E,))
        self.new_ui_frame_for_header.rowconfigure(0, minsize=self.new_var_row_height_for_header)
        self.new_ui_frame_for_header.rowconfigure(0, weight=0)
        self.new_ui_frame_for_header.columnconfigure(0, weight=1)
        
        parent = self.new_ui_frame_for_header
        # 比例
        ...
        self.new_ui_scrollbar_v.grid(row=0,column=0,sticky=(tk.N,tk.S))
...
class GameList_1(GameList_0):
    ...
    def new_func_set_row_height_for_header(self,row_height):
        if row_height > 0:
            ...
            self.new_ui_frame_for_header.rowconfigure(0,minsize=row_height)
            
            
            self.new_ui_header.destroy()
            
            self.new_ui_header = tk.Canvas(self.new_ui_frame_for_header,
                #bg="grey80",
                height =self.new_var_row_height_for_header,
                #width  =self.new_var_total_width,
                borderwidth=0,
